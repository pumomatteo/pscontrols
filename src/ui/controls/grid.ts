...
export class Grid extends VrControl
{
    ...
    constructor(element: HTMLElement, options?: GridOptions | null)
    {
        ...
        if (editButtonColumnsNumber == 0 && !options.hideEditButton)
            options.columns.unshift({ type: GridColumnTypeEnum.EditButton, field: "editButton" });
        //#endregion
        ...
        if (options.checkboxes != GridCheckboxModeEnum.None)
        {
            ...
            let textHTML = "<input id='" + element.id + "header_CheckboxColumn' class='vrCheckBox' type='checkbox'></input>"
                + "<label class='k-label vrCheckBoxLabel vr-checkboxColumnHeader-label' for='header_CheckboxColumn'></label>";
    ...
    {
        puma(this.element()).off("click", "." + className);
        puma(this.element()).on("click", "." + className, (e: any) =>
        {
    ...
    {
        let options = this.getOptions();
        if (options.groupable! && options.groupBy != null && options.checkboxes != GridCheckboxModeEnum.None && checkbox != null)
        {
                    ...
                    [
                        {
                            ...
                            value: "restoreOriginal", icon: IconClassLight.RotateLeft, onClick: (e) =>
                            {
                                ...
                                for (let checkBoxElement of Array.from<HTMLInputElement>(wndTableActionsContainer.find("input")))
                                {
                                    ...
                                    checkBox.checked(!this._originalHiddenColumnFields.includes(field));
                                }
                            }
                        },
                        {
                ...
                for (let jsonColumn of json.columns)
                {
                    ...
                    if (optionsColumn != null)
                    {
                        ...
                        if (updateDataSource)
                        {
                            if (jsonColumn.fitSpace !== true)
                            {
                                puma(this._divHeader).find("th[value='" + jsonColumn.field + "']")[0].style.width = jsonColumn.width + "px";
