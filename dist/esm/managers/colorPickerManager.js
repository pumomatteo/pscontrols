...
(function(global, factory) {
  ...
  var jscolor = (function() {
    var jsc = {
      ...
      register: function() {
        if (typeof window2 !== "undefined" && window2.document) {
          window2.document.addEventListener("DOMContentLoaded", jsc.pub.init, false);
        }
      },
      installBySelector: function(selector, rootNode) {
        rootNode = rootNode ? jsc.node(rootNode) : window2.document;
        if (!rootNode) {
      ...
      createEl: function(tagName) {
        var el = window2.document.createElement(tagName);
      ...
      node: function(nodeOrSelector) {
        ...
        if (typeof nodeOrSelector === "string") {
          ...
          var el = null;
          try {
            el = window2.document.querySelector(sel);
          } catch (e) {
            console.warn(e);
            return null;
          }
      ...
      isPassiveEventSupported: (function() {
        ...
        try {
          ...
          window2.addEventListener("testPassive", null, opts);
          window2.removeEventListener("testPassive", null, opts);
        } catch (e) {
        }
        return supported;
      })(),
      isColorAttrSupported: (function() {
        var elm = window2.document.createElement("input");
        if (elm.setAttribute) {
      ...
      triggerEvent: function(el, eventName, bubbles, cancelable) {
        ...
        if (typeof Event === "function") {
          ...
          ev = window2.document.createEvent("Event");
      ...
      removeClass: function(elm, className) {
        ...
        for (var i = 0; i < classNames.length; i += 1) {
          var repl = new RegExp(
            "^\\s*" + classNames[i] + "\\s*|\\s*" + classNames[i] + "\\s*$|\\s+" + classNames[i] + "(\\s+)",
            "g"
          );
      ...
      getCompStyle: function(elm) {
        var compStyle = window2.getComputedStyle ? window2.getComputedStyle(elm) : elm.currentStyle;
        if (!compStyle) {
          return {};
        }
        return compStyle;
      ...
      rgbaColor: function(r, g, b, a) {
        return "rgba(" + Math.round(r) + "," + Math.round(g) + "," + Math.round(b) + "," + Math.round((a === void 0 || a === null ? 1 : a) * 100) / 100 + ")";
      },
      linearGradient: (function() {
        function getFuncName() {
          var stdName = "linear-gradient";
          var prefixes = ["", "-webkit-", "-moz-", "-o-", "-ms-"];
          var helper = window2.document.createElement("div");
      ...
      getViewPos: function() {
        var doc = window2.document.documentElement;
        return [
          (window2.pageXOffset || doc.scrollLeft) - (doc.clientLeft || 0),
          (window2.pageYOffset || doc.scrollTop) - (doc.clientTop || 0)
        ];
      },
      getViewSize: function() {
        var doc = window2.document.documentElement;
        return [
          window2.innerWidth || doc.clientWidth,
          window2.innerHeight || doc.clientHeight
        ];
      },
      // r: 0-255
      // g: 0-255
      ...
      onControlPointerStart: function(e, target, controlName, pointerType) {
        ...
        registerDragEvents(window2.document, [0, 0]);
        if (window2.parent && window2.frameElement) {
          var rect = window2.frameElement.getBoundingClientRect();
          var ofs = [-rect.left, -rect.top];
          registerDragEvents(window2.parent.window.document, ofs);
        }
      ...
      pub: function(targetElement, opts) {
        ...
        this.setPreviewElementBg = function(color2) {
          ...
          for (var i2 = 0; i2 < backgrounds.length; i2 += 1) {
            let background = backgrounds[i2];
            bg.image.push(background.image);
            bg.position.push(background.position);
            bg.size.push(background.size);
            bg.repeat.push(background.repeat);
            bg.origin.push(background.origin);
          }
          var sty = {
        ...
        function drawPicker() {
          ...
          p.btnT.innerHTML = "";
          p.btnT.appendChild(window2.document.createTextNode(THIS.closeText));
          redrawPad();
          redrawSld();
          redrawASld();
          ...
          if (THIS.container === window2.document.body) {
            jsc.redrawPosition();
          } else {
        ...
        if (this.container === void 0) {
          this.container = window2.document.body;
        } else {
        ...
        if (jsc.isButton(this.targetElement)) {
          ...
          if (jsc.isButtonEmpty(this.targetElement)) {
            ...
            this.targetElement.appendChild(window2.document.createTextNode(" "));
    ...
    jsc.pub.init = function() {
      ...
      window2.document.addEventListener("mousedown", jsc.onDocumentMouseDown, false);
      window2.document.addEventListener("keyup", jsc.onDocumentKeyUp, false);
      window2.addEventListener("resize", jsc.onWindowResize, false);
      window2.addEventListener("scroll", jsc.onWindowScroll, false);
      jsc.pub.install();
  ...
  if (typeof window2.jscolor === "undefined") {
    window2.jscolor = window2.JSColor = jscolor;
  }
  return jscolor;
});
export {
