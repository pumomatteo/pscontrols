...
import { NotifierTypeEnum, NotifierPositionEnum, AnimationHideEnum, AnimationShowEnum, ColorSettings, puma, div, IconClassicRegular, icon, NotifierOptions, shadowRoot } from "../vr.js";
class Notifier {
  _options;
  _container;
  _arrow;
  ...
  constructor(text, options) {
    ...
    if (options.hideSettings.autoHide == null || options.hideSettings.autoHide === true) options.hideSettings.autoHide = 4e3;
  ...
  prepareNotification(text) {
    ...
    if (iconClass != null && options.icon !== false)
      puma(this._element).append(icon(iconClass, null, { css: "margin-right: 8px;" }));
  ...
  show(text) {
    ...
    switch (showSettings.animation) {
      case AnimationShowEnum.Default:
        puma(this._element).show(duration);
        break;
      case AnimationShowEnum.FadeIn:
        puma(this._element).fadeIn(duration);
        break;
      case AnimationShowEnum.SlideDown:
        puma(this._element).slideDown(duration);
        break;
      case AnimationShowEnum.None:
    ...
    if (hideSettings.autoHide !== false)
      ...
      puma(this._container).on("click", (e) => this.hide());
    if (text != null)
      puma(this._element).append("<label>" + text + "</label>");
  ...
  hide() {
    ...
    switch (hideSettings.animation) {
      case AnimationHideEnum.Default:
        puma(this._element).hide(duration);
        break;
      case AnimationHideEnum.FadeOut:
        puma(this._element).fadeOut(duration);
        break;
      case AnimationHideEnum.SlideUp:
        puma(this._element).slideUp(duration);
        break;
      case AnimationHideEnum.None:
        puma(this._element).hide();
        break;
      default:
        puma(this._element).hide(duration);
...
export {
  Notifier,
  NotifierCustomHtmlEvent,
  NotifierHideSettings,
  NotifierOnClickEvent,
  NotifierShowSettings
};
