...
import { SchedulerViewEnum, DayOfWeekEnum, ControlTypeEnum, puma, div, createButton, IconClassicLight, createDatePicker, DateFormatEnum, createSwitch, createButtonGroup, createComboBox, ComboBoxTypeEnum, SchedulerSlotElement, SchedulerNavigateActionEnum, confirm, SchedulerResource, showLoader, hideLoader, notify, NotifierTypeEnum } from "../vr.js";
class SchedulerOptions extends VrControlOptions {
  date;
  startTime;
  endTime;
...
class Scheduler extends VrControl {
  _dtpSchedulerDate;
  _swtGroupByDateResource;
  _btgMenuView;
  _btnExpand;
  _btnCollapse;
  _cmbInterval;
  ...
  constructor(element, options) {
    ...
    if (options.startTime == null) options.startTime = new Date((/* @__PURE__ */ new Date()).getFullYear(), (/* @__PURE__ */ new Date()).getMonth(), (/* @__PURE__ */ new Date()).getDate(), 8, 0);
    if (options.endTime == null) options.endTime = new Date((/* @__PURE__ */ new Date()).getFullYear(), (/* @__PURE__ */ new Date()).getMonth(), (/* @__PURE__ */ new Date()).getDate(), 19, 0);
    ...
    if (options.date == null) options.date = /* @__PURE__ */ new Date();
    ...
    this._timeColumnWidth = options.timeColumn ? 63 : 0;
    this._saturationData = void 0;
  ...
  drawToolbar() {
    ...
    this._btgMenuView = createButtonGroup({
      ...
      width: "auto",
      onSelect: (e) => {
        let maxResourceNumberBasedOnView = null;
        switch (Number(e.value)) {
          case SchedulerViewEnum.Day:
            maxResourceNumberBasedOnView = options.maxResourcesNumber.dayView;
            break;
          case SchedulerViewEnum.Week:
            maxResourceNumberBasedOnView = options.maxResourcesNumber.weekView;
            break;
          case SchedulerViewEnum.FourWeeks:
            maxResourceNumberBasedOnView = options.maxResourcesNumber.fourWeeksView;
            break;
        }
        if (maxResourceNumberBasedOnView != null && this.resources().length > maxResourceNumberBasedOnView) {
          confirm("Verranno visualizzate un numero di risorse maggiore<br /> del limite massimo impostato (" + maxResourceNumberBasedOnView + ").<br /> Continuare comunque?").then(
            () => this.view(Number(e.value)),
            () => e.sender.select([this.view()], false)
          );
        } else
          this.view(Number(e.value));
      }
  ...
  drawContent() {
      ...
      case SchedulerViewEnum.FourWeeks:
        {
          ...
          let numberOfRows2 = minutes / options.timeslotIntervalDuration;
          if (!this.groupWeekByDate()) {
            let date = new Date(options.startTime);
            for (let i = 0; i < numberOfRows2; i++) {
              let row = document.createElement("tr");
              table.appendChild(row);
              ...
              if (!options.timeColumn)
                thTime.style.cssText += "display: none;";
              let borderTopClass = i == 0 ? "" : i % 2 == 0 ? "borderTopSolid" : "borderTopDotted";
              let borderBottomClass = i == numberOfRows2 - 1 ? "borderBottomSolid" : "";
              let columnIndex = 0;
              for (let resource of this.resources()) {
    ...
    nowDate.setHours((/* @__PURE__ */ new Date()).getHours(), (/* @__PURE__ */ new Date()).getMinutes(), 0, 0);
  ...
  //#endregion
  //#region Properties
  view(view, drawScheduler = true, triggerChange = true) {
    if (view != null) {
      let options = this.getOptions();
  ...
  resources(resources, drawScheduler = true, triggerChange = true) {
    let options = this.getOptions();
    if (resources != null) {
      let maxResourceNumberBasedOnView = null;
      switch (this.view()) {
      ...
      if (maxResourceNumberBasedOnView != null && resources.length > maxResourceNumberBasedOnView) {
        confirm("Stai impostando un numero di risorse maggiore<br /> del limite massimo impostato (" + maxResourceNumberBasedOnView + ").<br /> Continuare comunque?").then(
          () => {
  ...
  timeslotInterval(timeslotIntervalDuration, drawScheduler = true, triggerChange = true) {
    let options = this.getOptions();
  ...
  daysOfWeekNames(names, drawScheduler = true) {
    let options = this.getOptions();
    if (names != null) {
  ...
  numberOfWorkDays(workDaysNumber, drawScheduler = true) {
    let options = this.getOptions();
  ...
  startTime(time, drawScheduler = true) {
    let options = this.getOptions();
    if (time != null) {
  ...
  endTime(time, drawScheduler = true) {
    let options = this.getOptions();
    if (time != null) {
  ...
  datasource(datasource) {
    if (datasource != null && this._slotElementList.length > 0) {
      ...
      for (let appointment of datasource) {
        ...
        if (daysDifference > 1) {
          ...
          for (let i = 1; i <= daysDifference; i++) {
            let newApp = UtilityManager.duplicate(appointment);
      ...
      let slotWidthList = Array.from(puma(this.container()).find(".vrSchedulerDivContent table tr:first-child td")).map((k) => {
        return puma(k).outerWidth();
      });
      let slotWidth = slotWidthList[0];
      let schedulerStartTime = new Date(options.startTime);
      let dictionaryResourceAppointments = datasource.vrGroupBy((k) => k.resourceId);
      for (let appointment of datasource) {
        if (appointment.positioned == true)
          continue;
        ...
        for (let apt of appointmentsOverlapped) {
          for (let slot of apt.slotsOccupied) {
            slot.columnPosition = [];
            ...
            for (let apt2 of appointmentsOverlapped) {
              if (apt2.id == apt.id)
                continue;
              for (let slot2 of apt2.slotsOccupied) {
            ...
            apt.overlappedList = overlappedList.vrDistinctBy((k) => k.id);
          }
        }
        appointmentsOverlapped.vrSortDesc("duration");
        for (let apt of appointmentsOverlapped) {
          let start = new Date(apt.start);
          this.adaptDateToTimeslotDuration(
            start,
            0
            /* Start */
          );
          ...
          let slotFiltered = apt.slotsOccupied[0];
          if (slotFiltered != null) {
            ...
            let occupiedSlotList = apt.slotsOccupied.map((k) => k.columnPosition);
            let index = 0;
            ...
            if (width <= 0)
              width = 1;
            slotWidthList.slice(0, slotFiltered.columnIndex).vrSum();
            let newSlotFilteredWidth = slotWidthList[0] * slotFiltered.columnIndex;
            ...
            puma(divAppointment).on("mousedown", (emd) => {
              ...
              puma(document).on("mousemove", (emm) => {
                ...
                if (yPositionMouse >= appointmentYPosition && yPositionMouse <= appointmentYPosition + appointmentHeight && xPositionMouse >= appointmentXPosition && xPositionMouse <= appointmentXPosition + appointmentWidth && slotToMove == null)
                  slotToMove = null;
                  ...
                  let sum2 = 0;
                  let slotColumnIndex2 = 0;
                  for (let slotsss of slotWidthList) {
                    sum2 += slotsss;
                    if (sum2 + this._timeColumnWidth > xPositionMouse)
                      break;
                    slotColumnIndex2++;
                  }
                  slotToMove = this._slotElementList.filter((k) => k.columnIndex == slotColumnIndex2 && k.rowIndex == Math.trunc((yPositionMouse - diffYMouseAppointment) / 33))[0];
                  let duration2 = Date.vrDifferenceBetweenDatesInMinutes(apt.end, apt.start);
                  let start2 = new Date(apt.start);
                  let end2 = new Date(start2).vrAddMinutes(duration2);
                  let options2 = this.getOptions();
                  if (options2.onMoveStart != null && !this._isMoving) {
                    this._isMoving = true;
                  ...
                  } else if (options2.onMoving != null) {
                    start2 = new Date(slotToMove.start);
                    end2 = new Date(slotToMove.start).vrAddMinutes(duration2);
                    ...
                    options2.onMoving(moveAppointmentEvent);
                    if (moveAppointmentEvent.isDefaultPrevented()) {
                      emm.preventDefault();
                      puma(document).unbind("mouseup");
                      puma(document).unbind("mousemove");
              ...
              puma(document).on("mouseup", () => {
                ...
                if (slotToMove != null) {
                  let duration2 = Date.vrDifferenceBetweenDatesInMinutes(apt.end, apt.start);
                  let start2 = new Date(slotToMove.start);
                  let end2 = new Date(start2).vrAddMinutes(duration2);
                  let appointmentDataSource = null;
                  if (apt.resourceId != null)
                    appointmentDataSource = this.datasource().filter((k) => k.id == divAppointment.id && k.resourceId == apt.resourceId)[0];
                  ...
                  if (options2.onMoveEnd != null) {
                    ...
                    moveAppointmentEvent.dataItem = appointmentDataSource;
                    moveAppointmentEvent.resourceId = slotToMove.resourceId;
                    options2.onMoveEnd(moveAppointmentEvent);
                    if (moveAppointmentEvent.isDefaultPrevented()) {
                      slotToMove = null;
                      return;
                    }
                  ...
                  let appointmentIndex = this.datasource().indexOf(appointmentDataSource);
                  appointmentDataSource = this.datasource()[appointmentIndex];
                  appointmentDataSource.start = start2;
                  appointmentDataSource.end = end2;
  ...
  setResizing(divAppointment, apt) {
    let options = this.getOptions();
    ...
    puma(divAppointment).on("mouseover", (e) => divResizable.style.display = "block");
    puma(divAppointment).on("mouseout", (e) => divResizable.style.display = "none");
    let pageY = null;
    let slotToResize;
    let schedulerOffsetTop = puma(this.getDivContent()).offset().top;
    let schedulerScrollOffset = this.getDivContent().scrollTop;
    ...
    puma(divResizable).on("mousedown", (e) => {
      ...
      puma(document).on("mousemove", (e2) => {
        if (divAppointment != null) {
          ...
          let appointmentIndex = this.datasource().indexOf(appointmentDataSource);
          appointmentDataSource = this.datasource()[appointmentIndex];
          e2.clientX - schedulerOffsetLeft;
          let yPositionMouse = e2.clientY + schedulerScrollOffset - schedulerOffsetTop;
      ...
      puma(document).on("mouseup", () => {
        ...
        if (slotToResize != null) {
          ...
          if (options2.onResizeEnd != null) {
            ...
            options2.onResizeEnd(moveAppointmentEvent);
            if (moveAppointmentEvent.isDefaultPrevented()) {
              slotToResize = null;
              return;
            }
  ...
  availabilities(availabilities) {
    if (availabilities != null) {
      ...
      for (let availability of availabilities) {
        ...
        let slotForAvailability = this._slotElementList.filter((k) => (availability.resourceId == k.resourceId || availability.resourceId == null) && Date.vrArePeriodsOverlapped(k.start, k.end, dateFrom, dateTo))[0];
  ...
  checkMandatorySchedulerDataFields(schedulerData) {
    let error = [];
  ...
  managePropertiesSchedulerData(divElement, schedulerData) {
  ...
  manageSaturationPercentage(saturation, divSaturation, spanPercentage) {
  ...
  internalShowSaturation() {
    for (let divSaturation of Array.from(puma(".vrSchedulerDivSaturation")))
      divSaturation.style.cssText += "display: flex;";
  ...
  //#endregion
  //#region Methods
  bounceAppointment(divElementId) {
  ...
  manageSchedulerClick() {
    puma(this.element()).on("click", ".vrSchedulerDivContent td, .vrSchedulerAppointment, .vrSchedulerAvailability", (e) => {
      let options = this.getOptions();
      if (!this.enabled())
        return;
    ...
    puma(this.element()).on("click", ".vrSchedulerDayResource", (e) => {
      if (e.button == 0) {
        let target = puma(e.currentTarget);
    ...
    puma(this.element()).on("click", ".vrSchedulerWeekDay", (e) => {
      if (e.button == 0) {
        let target = puma(e.currentTarget);
  ...
  //#endregion
  //#region Utilities
  findOverlappedAppointments(appointment, resourceAppointments) {
    let start = new Date(appointment.start);
    this.adaptDateToTimeslotDuration(
      start,
      0
    ...
    let appointmentsOverlapped = [];
    appointment.positioned = true;
    appointment.slotsOccupied = appointmentSlots;
    appointmentsOverlapped.push(appointment);
    for (let resAppointment of resourceAppointments) {
      if (resAppointment.positioned)
        continue;
      ...
      if (
        /*start.getDate() == resAppointmentStart.getDate() && end.getDate() == resAppointmentEnd.getDate()
        &&*/
  ...
  adaptDateToTimeslotDuration(date, dateType) {
...
class SchedulerTimeslotClickEvent extends SchedulerClickEvent {
}
class SchedulerAppointmentClickEvent extends SchedulerClickEvent {
  dataItem;
}
class SchedulerAvailabilityClickEvent extends SchedulerClickEvent {
  dataItem;
}
...
export {
  Scheduler,
  SchedulerAppointmentClickEvent,
  SchedulerAvailabilityClickEvent,
  SchedulerOptions,
  SchedulerTimeslotClickEvent
};
